...
# 補足
EC2インスタンスへのアクセス方法
- EC2 Instance Connectを利用したウェブブラウザ経由アクセス(本稿)
- SSH クライアント
...
WordPressのインストール方法
- ユーザデータを使用したEC2インスタンス起動時の自動インストール(本稿)
- EC2インスタンスにSSH等でアクセスし、インストール用コマンドを直接実行
Linuxインスタンスでの起動時のコマンド実行
  https://docs.aws.amazon.com/ja_jp/AWSEC2/latest/UserGuide/user-data.html

別パターンのアーキテクチャ
- EC2(Nginx+Rails) + DB(Postgres)
- EC2(Apache+Tomcat) + DB(Aurora MySQL)
- EC2インスタンスのAutoScalingの利用
- Amazon CloudFront + S3での静的コンテンツの配信
- Amazon ElastiCache(Redis/Memcached)の利用
