...
1か月で認証まわり+リファクタはちょうどいい。
Auth0使うと一気に「業務グレード」感が出る。
✅ **MUST DO**：

* ロール管理（admin/user）で表示制御を追加
* `.env`と環境依存を本番・開発で切り替えられるようにしとけ
* READMEにAuth0連携の構成図。**絶対やれ**（採用者そこ見る）

### 🔸**② AWS枠（4か月）**

贅沢タイム。
Serverless構成〜IaC（CloudFormation or CDK）までやれ。

✅ **進め方テンプレ**：

1. 月1：構成設計とIaC化（図描けるように）
2. 月2：機能実装（API Gateway + Lambda + DynamoDBくらいがカロリー的に◎）
3. 月3：S3 or CloudFrontで簡易フロント乗せ
4. 月4：CI/CD + CloudWatch Logs + メトリクス監視（CloudWatchで遊べ）

👓 「趣味でAWS触ってます」じゃなくて
👔 「ちょっとした業務システムなら作れるぞ」の雰囲気出せ。
**あとAWS使う理由書いとけよ。コスト感含めて。**

### 🔸**③ AI枠（4か月）**

これはセンス出る枠。
やる気が出ると急に**機械学習発表会**みたいになるけど、
\*\*「成果物としての使いやすさ」\*\*も入れておけ。

✅ **アイデア雑まとめ（お好きにどうぞ）**

* LLM×UI（Streamlit, Flask）で対話型プロンプトシステム
* SageMakerでfine-tuningしたミニ分類モデルをAPI化
* Bedrockでチャットボット（社内FAQ風）＋ログ収集・学習

🔬 CIFAR-10やるなら、転移学習だけで終わるな。**可視化と精度比較をしろ。**
あとDocker化してると最高。見た目で勝てる。
