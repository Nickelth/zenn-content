...
バッチ実行時刻がほかのジョブと競合し、タスクが失敗を繰り返していた。
過剰なタスク予約を防ぐためリトライは一度までとした。
...
データセンター移行のため、タスクスケジューラーおよびcronジョブを停止/再開する。
停止/再開は自動実行で行う。

タスクスケジューラーの停止はシステムに重大な影響を与える。
コマンドでの停止実行も、通常システム側に無効化される。(管理者権限でも不可能)

よって、以下の方式を採用した。

1. 各タスクの状態(有効or無効)のバックアップCSVを取得
2. 各タスクを一括停止
3. バックアップCSVをもとにタスクの状態を復元
